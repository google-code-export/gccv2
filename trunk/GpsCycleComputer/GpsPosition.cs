...
namespace GpsUtils
{
    ...
    public class GpsPosition
    {
        ...
        public bool IsHarwareOff
        {
            get { return (dwFlags == 0x00000001); }
        }

        /// Satellites used in the solution, returns Array of Satellites
        ...
        /// Satellites in view, returns Array of Satellites
        ...
        {
            // fix for version 2 (our struct), where some fields are replaced
            if(dwVersion == 2)
            {
                if( (dwValidFields & GPS_VALID_VERTICAL_DILUTION_OF_PRECISION) != 0 )
                {
                    return (int) ( 0.5 + flVerticalDilutionOfPrecision);
                }
                else
                    { return 0; }
            }
            ...
            if (dwSatellitesInView != 0)
            {
                ...
                {
                    if (rgdwSatellitesInViewSignalToNoiseRatio[index] > SNR)
                    { SNR = rgdwSatellitesInViewSignalToNoiseRatio[index]; } 
                }
            }
            return SNR;
        }
